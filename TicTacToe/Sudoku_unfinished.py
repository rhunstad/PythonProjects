...
header = canvas.create_text(225, 30, fill='#13B2E9', font='Calibri 30', text="Classic Sudoku",
                       anchor='center')
ones_turn = True
lost_trigger = 0
                ...
                if lost_trigger == 0:
                    ...
                    if lost_trigger == 1:
                        ...
                        header = canvas.create_text(750/2, 30, fill='#000000', font='Calibri 40',
                            text="Player %s loses!" % player_num, anchor='center')



                """
                global header, one, lost
                p = 2
                exes[key] = True
                if one:
                    p = 1
                    ...
                    if lost == 1:
                        ex(p, btn_x, btn_y)
                        canvas.delete(header)
                        header = canvas.create_text(225, 30, fill='#13B2E9', font='Arial 30', text="Player 1 loses!",
                           anchor='center')
                    one = False
                else:
                    if win():
                    ...
                    if lost == 1:
                        ...
                        ex(p, btn_x, btn_y)
                        canvas.delete(header)
                        header = canvas.create_text(225, 30, fill='#FF3F3F', font='Arial 30', text="Player 2 loses!",
                           anchor='center')
                    one = True
                if not win():
                    ex(p, btn_x, btn_y)
...
def setup():
    ...
    x = 60
    y = 60
    big_grey = rectangle(width=640, length=640, fill='#BBADA0', x=x, y=y)
    ...
    canvas.create_text(720, 250, fill='#776E65', font='Calibri 20', text="First to complete 3 X's\nin-a-row wins!",
                       anchor='w')
